...
class DemoDataseeder extends Seeder {
	...
	public function run() {
		...
		for ($i = 1; $i <= 5; $i++) {
			...
			/**
			 * Dummy Governorate Create code below
			 */
			$governorate = Governorate::create([
				'name' => $faker->company,
			]);
			$governorate->short_name = $this->recursive($governorate->name, 'Governorate');
			$governorate->save();
	...
	public static function recursive($yourString, $table_name = "") {
		...
		if (strpos($yourString, " ") === false) {
			...
			if (array_key_exists(3, $words) && $table_name == 'Governorate') {
				$first_char_after_space .= substr($words[4], 0, 1);
			}
			if ($table_name == 'Hospital') {
