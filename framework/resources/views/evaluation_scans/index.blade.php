...
@endsection
@section('breadcrumb')
    <li class="active">@lang('equicare.Evaluation_scan')</li>
@endsection
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                <div class="box-header">
                    <h4 class="box-title">@lang('equicare.Evaluation_scan')
                        @if(Auth::user()->hasDirectPermission('Create Evaluation_scans'))
                            <a href="{{ route('evaluation_scans.create') }}"
                        ...
                        <table id="data_table" class="table table-hover table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th> @lang('equicare.production date') </th>
                            ...
                            <tbody>
                                ...
                                @if (isset($evaluation))
                                    @foreach ($evaluation as $evaluations)
                                        ...
                                        <tr>
                                            <td> {{ $count }} </td>
                                            <td> {{ $evaluations->equipment->name ?? '-' }} </td>
                                            <td> {{ $evaluations->user->name ?? '-' }}</td>
                                            <td> {{ $evaluations->call_handle ? ucfirst($evaluations->call_handle) : '-' }} </td>
                                            <td> {{ $evaluations->working_status ? ucfirst($evaluations->working_status) : '-' }}
                                            </td>
                                            <td> {{ $evaluations->report_no ? sprintf('%04d', $evaluations->report_no) : '-' }}
                                            </td>
                                            {{-- <td>
...
ء                                            {{-- </td>
                                            ...
                                            <td>{{ $evaluations->equipment->production_date }}</td>
                                            ...
                                            {{-- <td> 
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <ul class="dropdown-menu custom" role="menu">
                                                        @if(Auth::user()->hasDirectPermission('Edit Evaluation_scans'))
                                                            <li>
                                                                <a href="{{ route('evaluation_scans.edit', $evaluations->id) }}"
                                                        ...
                                                        @endif --}}
                                                        {{-- @if(Auth::user()->hasDirectPermission('Delete Evaluation_scans'))
                                                            <li>
                                                                <a class=""
                                                ...
                                                {!! Form::open([
                                                    'url' => 'admin/evaluation_scans' . $evaluations->id,
                                                    'method' => 'DELETE',
                                                    'id' => 'form1',
